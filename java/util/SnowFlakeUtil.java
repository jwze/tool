...
public class SnowFlakeUtil {
    ...
    // 初始时间戳(纪年)
    private static final long INIT_EPOCH = 1751016616000L;
    ...
    // 最后12位，代表每毫秒内可产生最大序列号，即 2^4 - 1 = 15
    private static final long SEQUENCE_BITS = 4L;

    // 掩码（最低4位为1，高位都为0），主要用于与自增后的序列号进行位与，如果值为0，则代表自增后的序列号超过了15
    private static final long SEQUENCE_MASK = ~(-1L << SEQUENCE_BITS);

    // 同一毫秒内的最新序号，最大值可为 2^4 - 1 = 15
    private long sequence;

    // workId位需要左移的位数 4
    private static final long WORK_ID_SHIFT = SEQUENCE_BITS;

    // dataCenterId位需要左移的位数 4+3
    private static final long DATA_CENTER_ID_SHIFT = SEQUENCE_BITS + WORKER_ID_BITS;

    // 时间戳需要左移的位数 4+3+3
    private static final long TIMESTAMP_SHIFT = SEQUENCE_BITS + WORKER_ID_BITS + DATA_CENTER_ID_BITS;

    // dataCenterId，需要根据下发配置数据中心和服务器，下面workId同理，默认都是1
